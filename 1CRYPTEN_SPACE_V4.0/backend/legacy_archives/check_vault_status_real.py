...
print("--- STATUS DO CICLO (Firestore: vault_management/current_cycle) ---")
vault_doc = fs.collection("vault_management").document("current_cycle").get()
if vault_doc.exists:
    vault_data = vault_doc.to_dict()
    print(json.dumps(vault_data, indent=2))
else:
    print("Documento 'vault_management/current_cycle' não encontrado no Firestore.")
