...
async def run_reset():
    ...
    await firebase_service.update_banca_status(banca_reset)

    # 3. Limpar Slots Ativos (ID 1 a 4)
    logger.info("🧹 Limpando os 4 slots táticos...")
    for i in range(1, 5):
        logger.info(f"   Resetando Slot {i}...")
        await firebase_service.hard_reset_slot(i, reason="RESET_DAY_ONE")

    # 4. Limpar Trade History (Firestore)
    logger.info("📜 Deletando histórico de trades...")
    trade_docs = await asyncio.to_thread(firebase_service.db.collection("trade_history").get)
    deleted_hist = 0
    ...
    moon_docs = await asyncio.to_thread(firebase_service.db.collection("moonbags").get)
    deleted_moon = 0
    ...
    if firebase_service.rtdb:
         await asyncio.to_thread(firebase_service.rtdb.child("moonbag_vault").set, {})
    ...
    signal_docs = await asyncio.to_thread(firebase_service.db.collection("journey_signals").get)
    deleted_sig = 0
