...
async def check():
    ...
    docs = sovereign_service.db.collection("trade_history").order_by("timestamp", direction="DESCENDING").limit(5).get()
    for d in docs:
        t = d.to_dict()
        print(f"{t.get('timestamp')}: {t.get('symbol')} | {t.get('side')} | PnL: {t.get('pnl')} | ROI: {t.get('pnl_percent')}")

    print("\n--- SEARCH FARTCOIN ---")
    docs = sovereign_service.db.collection("trade_history").where("symbol", "==", "FARTCOINUSDT").get()
    if not docs:
        # Try without USDT
        docs = sovereign_service.db.collection("trade_history").where("symbol", "==", "FARTCOIN").get()
