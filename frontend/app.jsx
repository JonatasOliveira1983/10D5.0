...
// V110.171: ELITE BOOT SEQUENCE
        ...
        */
        ...
        const GridChartItem = ({ 
            ...
            const priceLinesRef = React.useRef({ entry: null, sl: null, tp: null, pivots: [] });
            ...
            const stUpSeriesRef = React.useRef(null);
            const stDownSeriesRef = React.useRef(null);

            // Sync with prop symbol if it changes, fallback to BTCUSDT if empty [V67.5]
            React.useEffect(() => {
                setLocalSymbol(symbol || "BTCUSDT");
            }, [symbol]);

            React.useEffect(() => {
                isFirstLoadRef.current = true; // Reset zoom on new symbol/tf
                const fetchKlines = async () => {
                    ...
                    try {
                        ...
                        const formatted = data.map(d => ({
                            time: (parseInt(d[0]) / 1000), // Removido offset manual para compatibilidade nativa
                            open: parseFloat(d[1]),
                            high: parseFloat(d[2]),
                            low: parseFloat(d[3]),
                        ...
                        const volumes = data.map(d => ({
                            ...
                            color: parseFloat(d[4]) >= parseFloat(d[1]) ? 'rgba(255, 255, 255, 0.3)' : 'rgba(239, 68, 68, 0.3)'
                        }));
                ...
                fetchKlines();
                const itv = setInterval(fetchKlines, 120000); // [V110.138] Drastically relaxed to 2m (fixes Bybit limit/klines crash)
                return () => clearInterval(itv);
            }, [localSymbol, localTimeframe]);

            // Sync Indicators Visibility
            React.useEffect(() => {
                if (sma8SeriesRef.current) sma8SeriesRef.current.applyOptions({ visible: eagleMode });
                if (sma21SeriesRef.current) sma21SeriesRef.current.applyOptions({ visible: eagleMode });
                if (stUpSeriesRef.current) stUpSeriesRef.current.applyOptions({ visible: eagleMode });
                if (stDownSeriesRef.current) stDownSeriesRef.current.applyOptions({ visible: eagleMode });
                if (chartInstance.current && chartInstance.current.volumeSeries) {
                    chartInstance.current.volumeSeries.applyOptions({ visible: true }); // [V110.137] Sempre visível
                }
            }, [eagleMode]);
                ...
                if (chartInstance.current) {
                    chartInstance.current.applyOptions({
                        watermark: {
                            ...
                            visible: true,
                            text: `${localSymbol} • ${localTimeframe === '60' ? '1H' : localTimeframe === '240' ? '4H' : localTimeframe + 'M'}`,
                            fontSize: 48,
                            horzAlign: 'center',
                            vertAlign: 'center',
                        }
            ...
            React.useEffect(() => {
                ...
                const series = chart.addCandlestickSeries({
                    upColor: '#10B981', downColor: '#EF4444', borderVisible: true, borderColor: '#000000', wickUpColor: '#10B981', wickDownColor: '#EF4444'
                });
                ...
                sma8SeriesRef.current = chart.addLineSeries({ color: '#f3f4f6', lineWidth: 2, title: 'SMA 8', visible: eagleMode });
                sma21SeriesRef.current = chart.addLineSeries({ color: '#94A3B8', lineWidth: 1, title: 'SMA 21', visible: eagleMode });
                stUpSeriesRef.current = chart.addLineSeries({ color: '#FFFFFF', lineWidth: 2, title: 'ST Up', lastValueVisible: false, priceLineVisible: false, visible: eagleMode });
                stDownSeriesRef.current = chart.addLineSeries({ color: '#EF4444', lineWidth: 2, title: 'ST Down', lastValueVisible: false, priceLineVisible: false, visible: eagleMode });
            ...
            React.useEffect(() => {
                if (candlestickSeries.current && historicalData.candles.length > 0) {
                    ...
                    if (chartInstance.current.volumeSeries) {
                        chartInstance.current.volumeSeries.setData(historicalData.volumes);
                        // [V110.137] Garantir volume visível e escalas ajustadas
                        chartInstance.current.volumeSeries.applyOptions({ visible: true }); 
                    }

                    // [V110.137] Ajuste de Zoom Inicial (Mostra os últimos ~120 candles)
                    ...
                    // Removed Extra Indicators (Pivot Points, SMA, SuperTrend) for 3-Line Minimalism
            ...
            // Update Price Lines (Entry/SL/Target 150%)
            React.useEffect(() => {
                if (!candlestickSeries.current) return;
                if (priceLinesRef.current.entry) candlestickSeries.current.removePriceLine(priceLinesRef.current.entry);
                if (priceLinesRef.current.sl) candlestickSeries.current.removePriceLine(priceLinesRef.current.sl);
                if (priceLinesRef.current.tp) candlestickSeries.current.removePriceLine(priceLinesRef.current.tp);
                priceLinesRef.current.entry = null;
                priceLinesRef.current.sl = null;
                priceLinesRef.current.tp = null;

                const activeSlot = slots.find(s => s.symbol && s.symbol.replace('.P','').replace('.p','') === localSymbol);
                
                let entryPoints = null;
                if(focusedAsset && focusedAsset.symbol.replace('.P','').replace('.p','') === localSymbol) {
                    const e = parseFloat(focusedAsset.entry);
                    const isLong = focusedAsset.side === 'LONG' || focusedAsset.side === 'Buy';
                    entryPoints = {
                        entry: e,
                        sl: parseFloat(focusedAsset.sl),
                        // [V110.126] Alvo de Emancipação 150% ROI
                        target: isLong ? e * 1.03 : e * 0.97
                    };
                } else if (activeSlot) {
                    const e = parseFloat(activeSlot.entry_price);
                    const isLong = activeSlot.side === 'Buy';
                    entryPoints = {
                        entry: e,
                ...
                if (!entryPoints || (!entryPoints.entry && !entryPoints.sl)) return;

                const { entry, sl, target } = entryPoints;

                if (entry > 0) {
                    priceLinesRef.current.entry = candlestickSeries.current.createPriceLine({
                        price: entry, color: '#FFFFFF', lineWidth: 1, lineStyle: 2, title: 'ENTRADA',
                    });
                }
                if (sl > 0) {
                    priceLinesRef.current.sl = candlestickSeries.current.createPriceLine({
                        price: sl, color: '#EF4444', lineWidth: 2, lineStyle: 0, title: 'STOP LOSS',
                    });
                }
                if (target > 0) {
                    priceLinesRef.current.tp = candlestickSeries.current.createPriceLine({
                        price: target, color: '#d9f99d', lineWidth: 2, lineStyle: 0, title: 'ALVO 150%',
                    });
                }
            }, [slots, localSymbol, focusedAsset]);

            return (
                <div className="flex flex-col h-full border border-white/10 bg-black/40 hover:border-primary/30 transition-all relative group overflow-hidden rounded-2xl shadow-2xl backdrop-blur-sm">
                    <div className="absolute top-3 left-4 z-20 flex items-center gap-2 pointer-events-none">
                        <span className="text-[12px] font-black text-white uppercase tracking-tighter drop-shadow-md">{localSymbol}</span>
                        <div className="w-1.5 h-1.5 rounded-full bg-white animate-pulse"></div>
                        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest opacity-80">{localTimeframe === '60' ? '1h' : localTimeframe === '240' ? '4h' : localTimeframe + 'm'}</span>
                    </div>
                    ...
                    {historicalData.candles.length === 0 && (
                        <div className="absolute inset-0 z-30 flex items-center justify-center bg-black/60 backdrop-blur-sm">
                            <div className="flex flex-col items-center gap-3">
                                ...
                                <span className="text-[9px] font-black text-white uppercase tracking-[0.3em] animate-pulse">Sincronizando Protocolo...</span>
                            </div>
                        </div>
                    )}
                    <div className="absolute top-2 right-2 z-40 flex items-center gap-2 opacity-30 group-hover:opacity-100 transition-opacity pointer-events-auto">
                        <select 
                            value={localSymbol} 
                            onChange={(e) => setLocalSymbol(e.target.value)}
                            className="bg-black/80 text-[9px] font-black text-white border border-white/10 rounded px-1 min-w-[70px] uppercase outline-none cursor-pointer"
                        >
                            <option value="BTCUSDT">BTCUSDT</option>
                            {slots.filter(s => s && s.symbol).map(s => (
                                <option key={s.id} value={s.symbol.replace('.P','').replace('.p','')}>{s.symbol.replace('.P','').replace('.p','')}</option>
                            ))}
                        </select>
                        <select 
                            value={localTimeframe} 
                            onChange={(e) => setLocalTimeframe(e.target.value)}
                            className="bg-black/80 text-[9px] font-black text-white border border-white/10 rounded px-1 min-w-[40px] uppercase outline-none"
                        >
                        ...
                        <button 
                            ...
                            className={`p-1 rounded border transition-all ${eagleMode ? 'bg-white/10 text-white border-white/20' : 'bg-white/5 text-gray-500 border-white/10'}`}
                            title={eagleMode ? "Eagle Vision: Analytic" : "Eagle Vision: Simple"}
                        >
                            <span className="material-icons-round text-[12px]">{eagleMode ? 'visibility' : 'visibility_off'}</span>
                        </button>
                        <span className="text-[10px] font-black text-white/50 uppercase tracking-tighter">#{index+1}</span>
                    </div>
                ...
                <div className="desktop-command-grid h-screen overflow-hidden flex bg-[#050505] text-white">
                    ...
                    <main className="flex-1 flex flex-col min-w-0 bg-[#050505] relative z-10">
                        ...
                        <div className="flex-1 p-6 flex flex-col gap-6 overflow-hidden relative">
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    <h2 className="text-sm font-black text-white uppercase tracking-[0.2em] font-display">Target Visualization</h2>
                                    <div className="flex bg-white/5 p-1 rounded-xl border border-white/10 gap-1 relative z-50 pointer-events-auto">
                                        {['1', '2', '4'].map(m => (
                                            <button 
                                                key={m} 
                                                onClick={(e) => {
                                                    e.preventDefault();
                                                    e.stopPropagation();
                                                    console.log("Setting Grid Mode:", m);
                                                    setGridMode(m);
                                                }}
                                                className={`w-10 h-8 flex items-center justify-center rounded-lg transition-all cursor-pointer ${gridMode === m ? 'bg-white text-black shadow-[0_0_15px_rgba(255,255,255,0.3)]' : 'text-gray-500 hover:text-white hover:bg-white/5'}`}
                                            >
                                                <span className="material-icons-round text-lg pointer-events-none">{m === '1' ? 'crop_square' : m === '2' ? 'view_column' : 'grid_view'}</span>
                                            </button>
                                        ))}
                                    </div>
                                </div>
                                <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10">
                                    ...
                                    <span className="text-[10px] font-black text-white uppercase">Live Engine Active</span>
                                </div>
                            </div>
