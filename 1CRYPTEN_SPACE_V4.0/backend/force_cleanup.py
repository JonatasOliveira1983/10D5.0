...
async def deep_clean():
    ...
    await asyncio.to_thread(firebase_service.db.collection('banca_status').document('status').set, status_doc)
    ...
    if firebase_service.rtdb:
        ...
        for target in targets:
            await asyncio.to_thread(firebase_service.rtdb.child(target).remove)
        
        await asyncio.to_thread(firebase_service.rtdb.child("banca_status").set, status_doc)
        await asyncio.to_thread(firebase_service.rtdb.child("system/bankroll").set, status_doc)
